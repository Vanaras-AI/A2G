...
from .signer import Signer, Signature
...
class ClientConfig:
    ...
    signing_key: Optional[str] = None  # HMAC-SHA256 signing key for DID auth
...
class A2gClient:
    ...
    async def connect(self) -> None:
        ...
        # Security: Warn if using unencrypted WebSocket
        if self.config.governance_url.startswith("ws://") and \
           "localhost" not in self.config.governance_url and \
           "127.0.0.1" not in self.config.governance_url:
            logger.warning("⚠️  A2G Security Warning: Using unencrypted ws:// connection. Use wss:// in production.")
        ...
        if self.config.signing_key:
            ...
            extra_headers["X-Agent-DID"] = self.config.agent_did
            extra_headers["X-Timestamp"] = sig.timestamp
            extra_headers["X-Nonce"] = sig.nonce
            extra_headers["X-Signature"] = sig.hash
        ...
        if self.config.signing_key:
            ...
            if "params" in msg:
                if "context" not in msg["params"] or msg["params"]["context"] is None:
                    msg["params"]["context"] = {}
                msg["params"]["context"]["signature"] = sig.to_dict()
        
        response = await self._request(msg)
